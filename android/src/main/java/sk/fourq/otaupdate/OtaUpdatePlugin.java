...
public class OtaUpdatePlugin implements FlutterPlugin, ActivityAware, EventChannel.StreamHandler, PluginRegistry.RequestPermissionsResultListener {
        ...
        try {
            ...
            fileDownloader.download()
                    ...
                    .subscribe(progress -> {
                        ...
                        if (progressSink != null && progress > 0) {
                        ...
                        Log.d(TAG, "OTA UPDATE FAILURE: " + error.getMessage());
                        Message message = new Message();
                        Bundle data = new Bundle();
     ...
     * @param messenger BinaryMessanger for communication with dart
     */
...
//                        long bytesTotal = data.getLong(BYTES_TOTAL);
                        long bytesTotal = data.getLong(PROGRESS);
                        progressSink.success(Arrays.asList("" + OtaStatus.DOWNLOADING.ordinal(), "" + PROGRESS));
                    }
                }
            }
        };
        final EventChannel progressChannel = new EventChannel(messenger, "sk.fourq.ota_update");
    ...
    private boolean isNetworkWifi(Network network) {
    ...
    void bindProcessToNetwork() {
        try {
            ...
            Log.d(TAG, "BIND NETWORK SUCCESS ");

        } catch (Exception e) {
            Log.d(TAG, "BIND NETWORK ERROR: " + e.getMessage());
        }
